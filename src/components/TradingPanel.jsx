// src/components/chart/TradingPanel.jsx
    ...
    return (
        <div className="w-72 bg-[#1e2329] border-l border-[#2b3139] flex flex-col p-3 gap-3 z-30" style={{ cursor: 'default' }}>
          ...
          <div className="space-y-4 mt-2">
            ...
            {tradeMode === 'grid' && (
              <div className="space-y-3">
                <div className="flex bg-[#2b3139] p-0.5 rounded-sm">
                    <button onClick={() => setGridType('spot')} className={`flex-1 py-1 text-xs rounded-sm transition-all ${gridType === 'spot' ? 'bg-[#474d57] text-white font-bold' : 'text-[#848e9c]'}`}>現貨網格</button>
                    <button onClick={() => setGridType('futures')} className={`flex-1 py-1 text-xs rounded-sm transition-all ${gridType === 'futures' ? 'bg-[#474d57] text-white font-bold' : 'text-[#848e9c]'}`}>合約網格</button>
                </div>
                ...
                {gridType === 'futures' && (<div><div className="text-xs text-[#848e9c] mb-1">調整槓桿倍數</div><div className="flex items-center gap-2"><input type="range" min="1" max="125" value={leverage} onChange={e => setLeverage(Number(e.target.value))} className="flex-1 h-1 bg-[#474d57] rounded-lg appearance-none cursor-pointer accent-[#f0b90b]" /><span className="text-[#f0b90b] text-sm font-bold w-8 text-right">{leverage}x</span></div></div>)}
                <div>
            ...
            {tradeMode !== 'grid' && (
                <div>
                    {/* 🔥 新增：合約模式下的「按價值/按本金」切換 */}
                    {tradeMode === 'futures' && amountType === 'usdt' && (
                        <div className="flex gap-2 mb-2 text-[10px]">
                            <button 
                                onClick={() => setFuturesInputMode('value')} 
                                className={`px-2 py-1 rounded transition-colors ${futuresInputMode === 'value' ? 'bg-[#474d57] text-[#f0b90b] border border-[#f0b90b]' : 'bg-[#2b3139] text-[#848e9c] border border-transparent'}`}
                            >
                                按價值買入
                            </button>
                            <button 
                                onClick={() => setFuturesInputMode('cost')} 
                                className={`px-2 py-1 rounded transition-colors ${futuresInputMode === 'cost' ? 'bg-[#474d57] text-[#f0b90b] border border-[#f0b90b]' : 'bg-[#2b3139] text-[#848e9c] border border-transparent'}`}
                            >
                                按本金買入
                            </button>
                        </div>
                    ...
                    <div className="flex justify-between items-center text-xs text-[#848e9c] mb-1">
                        <span>
                            {tradeMode === 'futures' && amountType === 'usdt' 
                                ? (futuresInputMode === 'value' ? '倉位價值 (Size)' : '投入本金 (Cost)') 
                                : '數量'}
                        </span>
                        <div className="flex gap-2 text-[10px]">
                            <span onClick={() => setAmountType('usdt')} className={`cursor-pointer ${amountType === 'usdt' ? 'text-[#f0b90b] font-bold' : ''}`}>USDT</span>/
                            <span onClick={() => setAmountType('coin')} className={`cursor-pointer ${amountType === 'coin' ? 'text-[#f0b90b] font-bold' : ''}`}>{symbol.replace('USDT', '')}</span>
                        </div>
                    </div>
                    <div className="relative">
                        <input 
                            ...
                            className="w-full bg-[#2b3139] border border-[#474d57] rounded-sm p-2 text-right text-white text-sm focus:border-[#f0b90b] outline-none" 
                        />
                        <span className="absolute left-2 top-2 text-xs text-[#848e9c]">
            ...
            {tradeMode === 'futures' && (
                <div>
                    <div className="flex justify-between text-xs text-[#848e9c] mb-1"><span>槓桿倍數</span> <span className="text-[#f0b90b]">{leverage}x</span></div>
                    <input type="range" min="1" max="125" value={leverage} onChange={e => setLeverage(Number(e.target.value))} className="w-full h-1 bg-[#474d57] rounded-lg appearance-none cursor-pointer accent-[#f0b90b]" />
                </div>
            )}
            ...
            <button onClick={handleTrade} className={`w-full py-3 rounded-sm font-bold text-sm shadow-md mt-4 transition-transform active:scale-95 ${tradeMode === 'grid' ? 'bg-[#f0b90b] hover:bg-[#d9a506] text-black' : (side === 'long' ? 'bg-[#089981] hover:bg-[#067a65] text-white' : 'bg-[#F23645] hover:bg-[#c92533] text-white')}`}>
              {tradeMode === 'grid' 
                ? `創建${gridType === 'spot' ? '現貨' : '合約'}網格策略` 
                : tradeMode === 'spot' 
                    ? (side === 'long' ? '買入 (Buy)' : '賣出 (Sell)') 
                    : (side === 'long' ? '做多 (Long)' : '做空 (Short)')
              } 
              {tradeMode !== 'grid' && ` (${orderType === 'limit' ? '限價' : '市價'})`}
            </button>
          </div>
        </div>
    );
