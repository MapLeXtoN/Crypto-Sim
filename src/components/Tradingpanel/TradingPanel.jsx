...
import { ChevronDown, ChevronUp, Settings2, Info, DollarSign, Coins } from "lucide-react";
...
const TradingPanel = ({
    ...
    useEffect(() => {
        setGridLowerPrice("");
        setGridUpperPrice("");
    }, [symbol, setGridLowerPrice, setGridUpperPrice]);
    ...
    const estimatedLiqPrice = useMemo(() => {
        ...
        if (tradeMode === "grid" && gridType === "futures" && !amount) {
            return "--";
        }
    ...
    }, [amount, amountType, currentPrice, symbol, feeSettings, selectedExchange, tradeMode, side]);

    const onSubmit = () => {
        const advancedParams = { gridSpacingType, triggerPrice, stopLoss, takeProfit, previewGrid };
                ...
                {tradeMode === "grid" ? (
                    ...
                    <div className="flex flex-col gap-4 mt-2">
                        <div className="flex bg-[#2b3139] rounded p-1">
                            <button onClick={() => { setSide("long"); setAmount(""); }} className={`flex-1 py-2 rounded text-sm font-bold transition-all ${side === "long" ? "bg-[#089981] text-white" : "text-[#848e9c]"}`}>
                                {tradeMode === "futures" ? "做多 (Long)" : "買入 (Buy)"}
                            </button>
                            <button onClick={() => { setSide("short"); setAmount(""); if(tradeMode === "spot") setAmountType("coin"); }} className={`flex-1 py-2 rounded text-sm font-bold transition-all ${side === "short" ? "bg-[#F23645] text-white" : "text-[#848e9c]"}`}>
